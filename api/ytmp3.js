...
const apis = {
  ...
  api2: (url, quality) => axios.get(`https://api.ryzumi.vip/api/downloader/ytmp3?url=${url}&quality=${quality}`),
  api3: (url, quality) => axios.get(`https://api.akane.web.id/api/downloader/youtube?url=${url}&type=video&quality=${quality}`),
};

async function ytmp3(url, quality = null, selectedApi = null) {
  try {
    if (selectedApi) {
    ...
    throw new Error("Download audio gagal: " + e.message);
  }
}
